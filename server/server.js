...
if (!groqApiKey) {
  console.warn(
    "WARNING: GROQ_API_KEY is not defined in the environment. Chatbot requests will fail.",
  );
}
...
const mongoUri =
  process.env.MONGO_URI || "mongodb://127.0.0.1:27017/varta_assistant";
mongoose
  .connect(mongoUri)
...
app.post("/api/widget/onboard", async (req, res) => {
  ...
  console.log(
    `[WIDGET] [ONBOARD] Request received to onboard visitor: "${name}" (${profession}) | Goal: "${goal}"`,
  );

  try {
    if (!name || !profession || !goal) {
      console.warn(
        `[WIDGET] [ONBOARD] [BAD REQUEST] Missing onboarding fields.`,
      );
      return res
        .status(400)
        .json({ error: "Name, profession, and goal are all required." });
    }
    ...
    console.log(
      `[WIDGET] [ONBOARD] [SUCCESS] Onboarded "${savedVisitor.name}". Created Conversation ID: ${savedConversation._id}`,
    );
    ...
    console.error(
      "[WIDGET] [ONBOARD] [ERROR] Failed to complete onboarding:",
      error,
    );
    return res
      .status(500)
      .json({ error: "Failed to complete visitor onboarding." });
  }
});
...
app.get("/api/widget/history/:visitorId", async (req, res) => {
  ...
  console.log(
    `[WIDGET] [HISTORY] Fetching history logs for visitor ID: ${visitorId}`,
  );

  try {
    if (!mongoose.Types.ObjectId.isValid(visitorId)) {
      console.warn(
        `[WIDGET] [HISTORY] [BAD REQUEST] Invalid visitor ID: ${visitorId}`,
      );
    ...
    if (!visitor) {
      console.warn(
        `[WIDGET] [HISTORY] [NOT FOUND] Visitor not found: ${visitorId}`,
      );
      return res.status(404).json({ error: "Visitor not found." });
    }

    // Find the latest conversation for this visitor
    const conversation = await Conversation.findOne({ visitorId }).sort({
      createdAt: -1,
    });
    if (!conversation) {
      console.log(
        `[WIDGET] [HISTORY] No active conversation session found for visitor "${visitor.name}".`,
      );
    ...
    console.log(
      `[WIDGET] [HISTORY] [SUCCESS] Retrieved ${messages.length} messages for visitor "${visitor.name}".`,
    );
    ...
    console.error(
      "[WIDGET] [HISTORY] [ERROR] Failed to fetch chat history:",
      error,
    );
    return res
      .status(500)
...
app.post("/api/widget/chat", async (req, res) => {
  ...
  console.log(`\n================== [CHAT PIPELINE START] ==================`);
  console.log(
    `[WIDGET] [CHAT] New message received from visitor ID: ${visitorId}`,
  );
  ...
  try {
    if (!visitorId || !conversationId || !text) {
      console.warn(
        `[WIDGET] [CHAT] [BAD REQUEST] Missing required body fields.`,
      );
      console.log(
        `================== [CHAT PIPELINE END] ==================\n`,
      );
      return res
        .status(400)
        .json({ error: "visitorId, conversationId, and text are required." });
    }
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: "Invalid visitor or conversation ID." });
    }
    ...
    if (!visitor) {
      console.warn(
        `[WIDGET] [CHAT] [NOT FOUND] Visitor profile not found for ID: ${visitorId}`,
      );
      console.log(
        `================== [CHAT PIPELINE END] ==================\n`,
      );
    ...
    console.log(
      `[WIDGET] [CHAT] Loaded Context -> Name: "${visitor.name}" | Profession: "${visitor.profession}" | Goal: "${visitor.goal}"`,
    );
...
`;
    ...
    console.log(
      `[WIDGET] [CHAT] Compiled system instruction instructions for AI.`,
    );
    ...
    if (groqApiKey) {
      try {
        console.log(
          `[WIDGET] [CHAT] Dispatching prompt to Groq model: "${config.GROQ_MODEL}"...`,
        );
        ...
        aiReplyText =
          "I encountered an API configuration error. Please ensure the Groq API key is valid.";
      }
    } else {
      console.log(
        `[WIDGET] [CHAT] [DEMO MODE] No GROQ_API_KEY. Using mock placeholder.`,
      );
    ...
    console.error(
      "[WIDGET] [CHAT] [ERROR] General exception in chat handler:",
      error,
    );
    console.log(`================== [CHAT PIPELINE END] ==================\n`);
...
app.get("/api/analytics", async (req, res) => {
  console.log(
    `[ADMIN] [ANALYTICS] Request to fetch dashboard analytics overview.`,
  );
  try {
    ...
    console.log(
      `[ADMIN] [ANALYTICS] [SUCCESS] Computed stats: Visitors=${totalVisitors}, Conversations=${totalConversations}, Messages=${totalMessages}`,
    );
    ...
    console.error(
      "[ADMIN] [ANALYTICS] [ERROR] Failed to fetch analytics:",
      error,
    );
    return res
      .status(500)
...
app.get("/api/conversations", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `[ADMIN] [CONVERSATIONS] [SUCCESS] Retrieved ${conversations.length} total conversation headers.`,
    );
    return res.status(200).json(conversations);
  } catch (error) {
    console.error(
      "[ADMIN] [CONVERSATIONS] [ERROR] Failed to list conversations:",
      error,
    );
...
app.get("/api/conversations/:id", async (req, res) => {
  const { id } = req.params;
  console.log(
    `[ADMIN] [CONVERSATIONS] Request to load detailed log for conversation ID: ${id}`,
  );

  try {
    if (!mongoose.Types.ObjectId.isValid(id)) {
      console.warn(
        `[ADMIN] [CONVERSATIONS] [BAD REQUEST] Invalid conversation ID format.`,
      );
    ...
    if (!conversation) {
      console.warn(
        `[ADMIN] [CONVERSATIONS] [NOT FOUND] Conversation logs not found for ID: ${id}`,
      );
    ...
    console.log(
      `[ADMIN] [CONVERSATIONS] [SUCCESS] Loaded ${messages.length} messages for transcript with "${conversation.visitorId?.name || "Anonymous"}".`,
    );
    ...
    console.error(
      "[ADMIN] [CONVERSATIONS] [ERROR] Failed to fetch conversation logs:",
      error,
    );
    return res
      .status(500)
      .json({ error: "Failed to retrieve conversation logs." });
  }
});
  ...
  res.sendFile(path.join(frontendBuildPath, "index.html"), (err) => {
    if (err) {
      ...
      res
        .status(200)
        .send(
          "SupportGPT Backend running. Frontend dist not detected. Use npm run dev for dev environments.",
        );
    }
  });
});
...
app.listen(PORT, () => {
  console.log(`=======================================================`);
  console.log(` SupportGPT IS LIVE ON PORT ${PORT}`);
  console.log(` Running API server and loading static assets...`);
  console.log(
    ` Default Admin Portal Password configured: "${config.ADMIN_PASSWORD}"`,
  );
  console.log(`=======================================================`);
});
